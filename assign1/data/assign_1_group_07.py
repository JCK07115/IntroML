...
"""
@params:
        k_set   - opt - list of k values to use as neighbours in KNN model, defaults to [5]
...
def classify(k_set=[5], metric="Euclidean"):
    ...
    for k in k_set:
        ...
        # predict
        test_df['pred_labels_'+str(k)+'_'+name] = model.predict(test_df[['x1', 'x2']])
...
"""
def draw_plot(train_errs, test_errs, k_set, metric="Euclidean", rows=2, cols=1):
    ...
    for i in range(len(k_set)):
        ...
        axs[i].set(xlabel="$x_1$", ylabel="$x_2$", title = "k=" + str(k_set[i]) + ", Training Error: " +
                    str(train_errs[i]) + ", Testing Error Rate: " + str(test_errs[i]))
...
def Q1_results():
    ...
    plt.savefig('./plots/Q1.png', format='png')
    # plt.show()
...
def Q2_results():
    ...
    train_errs_manh, test_errs_manh = classify([k_set[ind_min_test_err_eucl]], "Manhattan")
    global mean_manh_test_err
    mean_manh_test_err = mean(test_errs_manh)

    # plot results and save
    draw_plot(train_errs_manh, test_errs_manh, [k_set[ind_min_test_err_eucl]], metric="Manhattan")
    plt.savefig('./plots/Q2.png', format='png')
    # plt.show()
...
def Q3_results():
    ...
    plt.savefig('./plots/Q3.png', format='png')
    # plt.show()
...
def diagnoseDAT(Xtest, data_dir):
    ...
    hp_tuning = grid_search(train_df[['x1', 'x2']], train_df['true_label'], KNeighborsClassifier(), grid_params)
    model = KNeighborsClassifier(metric=hp_tuning['metric'], n_neighbors=hp_tuning['n_neighbors'], weights=hp_tuning['weights'])
    model.fit(train_df[['x1', 'x2']], train_df['true_label'])
    ytest = model.predict(Xtest)

    
    print('train_error: ', round( 1 - model.score(train_df[['x1', 'x2']], train_df['true_label']), 4))    
    print('test_error: ', round(1 - model.score(Xtest, test_df['true_label']), 4))
